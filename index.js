...
contributionsCard.innerHTML = `<h4>${totalContributions.toLocaleString('en-US')}</h4>`
...
raisedCard.innerHTML = ` <h4> $${totalRaised.toLocaleString('en-US')}</h4>`;
...
gamesCard.innerHTML = `<h4>${totalGames}</h4>`;
...
const unfundedGames = GAMES_JSON.filter((game) => {return game.pledged < game.goal});
const unfundedGamesCount = unfundedGames.reduce((acc, game) => acc + 1, 0);
...
descr_div.innerHTML = `<p>${displayStr}<p>`

descriptionContainer.appendChild(descr_div)
...
const mostPledged = document.createElement("div")
mostPledged.innerHTML = `<h4>${mostFunded.name}<h4>`
firstGameContainer.appendChild(mostPledged)

// do the same for the runner up item
const runnerUp = document.createElement("div")
runnerUp.innerHTML = `<h4>${secondFunded.name}<h4>`
